   ...
   stage('Login') {
	...
	// sh 'docker push  $REGISTRY/$HARBOR_NAMESPACE/$APP_NAME:jenkins-nodejs'
      ...
      stage('Deploy Dev') {
		...
		&& helm upgrade --install $RELEASE_DEV ./node-app-chart --set image.tag=${IMAGE_TAG_DEV}"'
      ...
      stage('Deploy Prod') {
		...
		&& helm upgrade --install $RELEASE_PROD ./node-app-chart --set image.tag=${IMAGE_TAG_PROD}"'
            }
          }
        }     
      }
